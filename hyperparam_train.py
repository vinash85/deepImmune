    ...
    with tqdm(total=num_batches_per_epoch) as t:
        for i, (features, all_labels) in zip(range(num_batches_per_epoch), dataloader):
            ...
            if i % params.save_summary_steps == 0:
                ...
                # print(survival[0, :])
                summary_batch = {
                metric: metrics[metric](output_batch[:, 0], survival) if metric == 'c_index' else metrics[metric](
                    output_batch[:, -1], labels_san_survival[:, -1])
...
def train_and_evaluate(embedding_model, outputs, datasets, embedding_optimizer, outputs_optimizer, metrics, params,
    ...
    for epoch in range(params.num_epochs):
        ...
        val_metrics_all = [evaluate(embedding_model, outputs, dataloader['val'], metrics, params) for dataloader in
                           datasets]
...
def setup_and_train(args):
    ...
    embedding_model = net.EmbeddingNet(
        net.ConvolutionBlock, input_size, out_channels_list=params.out_channels_list,
